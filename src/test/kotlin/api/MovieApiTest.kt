...
class MovieApiTest(
    ...
    fun `영화 목록 조회 응답에 영화와 상영 정보가 올바르게 포함된다`() {
        client.get().uri("/api/movies")
            ...
            .jsonPath("$.movies[0].title").isEqualTo("탑건: 매버릭")
            .jsonPath("$.movies[0].runningTimeMinutes").isEqualTo(130)
    ...
    private fun reservationBody(screeningId: Long, seats: List<String>) =
        ReservationRequest(
